...
BatchPIRClient::BatchPIRClient(unsigned int tree_height, unsigned int children, const BatchPirParams &params)
    : batchpir_params_(params), is_cuckoo_generated_(false), is_map_set_(false), tree_height_(tree_height), children_(children)
{
...
void BatchPIRClient::prepare_pir_clients()
{
    ...
    for (int i = 0; i < num_client; i++)
    {
        ...
        PirParams params(max_bucket_size, entry_size, num_dbs, tree_height_, children_,  batchpir_params_.get_seal_parameters(), dim_size);
        if (i == 0)
        {
