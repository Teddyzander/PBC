...
public:
    ...
    void set_client_keys(uint64_t client_id, std::pair<seal::GaloisKeys, seal::RelinKeys> keys);
    void get_client_keys();
    vector<RawDB> get_buckets();
    PIRResponseList generate_response(uint64_t client_id, vector<PIRQuery> queries);
...
private:
    ...
    PIRResponseList merge_responses(vector<PIRResponseList>& responses, uint64_t client_id);
